...
* **New: every admin screen redesigned.** Settings, Blocked, Failed syncs and
  Abandoned carts now share one application shell — a compact top bar carrying the
  connection state, a sidebar that owns navigation, and a single column of cards.
  The four screens previously each drew their own header and their own tables.
* Settings is now six sections behind a sidebar nav, with an overview strip
  (store, six figures, setup guide, granted permissions), a sticky unsaved-changes
  bar with Discard, and a search that jumps to any setting by name.
* Failed syncs moves its cause rail into the sidebar, so the counts that tell you
  which single fix clears most of the pile are visible without scrolling.
* Abandoned carts shows where open carts stopped as a bar per checkout step rather
  than a row of equal tiles, so the drop-off has a shape.
* Blocked splits into its two halves — the refusals record and your own list — with
  the compose form rebuilt as a card.
* **New: a date-range picker on Abandoned carts.** The two browser date fields
  are now one control — preset shortcuts (Today, Last 7 days, This month, All
  time) beside a two-month calendar, so "last 30 days" is one press instead of
  two date entries. Month names, weekday order and "today" all follow your
  WordPress settings rather than the browser's.
* All four screens work on a phone: the sidebar becomes a swipeable pill nav, and
  it is the same nav rather than a second hidden copy, so it stays reachable by
  keyboard.
* Icons are now bundled (Phosphor, MIT) instead of loaded from a CDN, so no admin
  page makes an external request. See NOTICE.md.
* Accessibility: the primary accent is used at a darker step wherever white text
  sits on it, so button labels clear WCAG AA. Contrast for every text pair is
  measured and recorded in DESIGN.md.
* Update: courier marks show the carrier's favicon where a domain is known,
  falling back to the monogram tile.
* Dev: `design-tokens.json` and `design-preview.html` are now generated by
  `bin/make-design-tokens.php` rather than hand-maintained, with a test that fails
  if either drifts from the stylesheet.
